...
loadTexture name path = do
  ...
  case takeExtension path of
    ".gif" -> case decodeGifImages imgData of
      Left err -> logError ("could not load image " ++ path ++ ": " ++ err)
        >> return (name, M.empty)
      Right images -> do
    ...
    ext -> do
      ...
      either
        (\err -> logError ("could not load image " ++ path ++ ": " ++ err)
          >> return (name, M.empty)
        )
...
handleImage img = case img of
 ...
 where
  ...
  imgToTexture image format internalFormat = do
    ...
    unsafeWith (imageData image) $ \ptr -> GL.texImage2D
      Texture2D
      NoProxy
      0
      internalFormat
      (TextureSize2D (fromIntegral $ imageWidth image)
