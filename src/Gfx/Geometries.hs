...
triVertexArray verts points = reverse $ foldl
  (\va (p1, p2, p3) ->
    genericIndex verts p3 : genericIndex verts p2 : genericIndex verts p1 : va
  )
  []
  points

lineVertexArray :: [v] -> [(Integer, Integer)] -> [v]
lineVertexArray verts points = reverse $ foldl
  (\va (p1, p2) -> genericIndex verts p1 : genericIndex verts p2 : va)
  []
  points

lineVertices :: GLfloat -> [Vertex3 GLfloat]
lineVertices size = let s = size / 2 in [Vertex3 0 (-s) 0, Vertex3 0 s 0]
...
sphereTriangles segments =
  let
    ...
    intraRingTriangles = do
      ...
      let
        upper =
          (o + s, (mod s segments + 1) + (r * segments), mod s segments + 1 + o)
      let lower =
...
sphereWireframe segments =
  let
    ...
    bottomLines = fmap (\s -> (s + ((rings - 1) * segments), bottomCentre))
                       [1 .. segments]
  in
